        ...
        if(requestCode == 10000) {
            if(resultCode == Activity.RESULT_OK) {
                ...
                if(existsID.length() == 0)
                    Toast.makeText(this.cordova.getActivity(), "registration success!",  Toast.LENGTH_SHORT).show();
                else
                    Toast.makeText(this.cordova.getActivity(), "duplicated user!",  Toast.LENGTH_SHORT).show();                    
            } else {                
                FacePlugin.callbackContext.error("registration canceled!");                
                Toast.makeText(this.cordova.getActivity(), "registration canceled",  Toast.LENGTH_SHORT).show();
            }
